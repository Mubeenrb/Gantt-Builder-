...
- **Resize any table column** by dragging the edge of its header (including Task Name, which isn't fixed-width) — hover the right edge of a header cell for a blue drag handle. Widths are remembered per project.
...
- **Export as Image** also opens a column picker — the same field list as the Excel one, but it controls which fields show up as extra text columns beside the task names in the picture (the bars/milestones/arrows themselves are always drawn). It defaults to whatever columns are currently visible in your on-screen table, since the point of an image export is to look like a clean chart, not a full data dump — pick fewer for a tidier slide, or add more (Budget, Assignee, custom fields, etc.) if the picture needs to stand alone. There's also a checkbox to leave out the "Generated on {date}" line under the title. Produces a single clean PNG — task names, columns, bars, milestones, dependency arrows, today marker, and (if the Critical Path toggle is on) the highlighted critical chain, all on a plain white background with no toolbar/table chrome. This is the one to use for a slide deck — the Excel export's colored-cell "Gantt" sheet is a real spreadsheet meant for filtering/editing in Excel, not for pasting into a presentation. Just Insert → Picture in PowerPoint (or paste directly).
  - **Timeline detail** at the top of that same dialog controls how fine-grained the exported chart's columns are (Day/Week/Month/Quarter/Year), independently of whatever zoom the on-screen chart happens to be at. Leave it on **Auto (recommended)** and it picks the coarsest level that still keeps the picture readable and slide-friendly: 30 days or less exports at Day detail, up to ~3 months at Week, and anything from a few months up to ~2 years — the typical CAPEX plan length — exports at **Month** (one clean "Jul"/"Aug"/"Sep" gridline per month, no W1-W4 sub-ticks), with Quarter/Year kicking in for multi-year plans. Week and Day are still there to pick manually if you specifically want that finer detail and don't mind a wider, busier export.
  - At **Quarter** detail, each quarter is split into its 3 calendar months (its own gridline + "Jan"/"Feb"/"Mar" label), with the quarter itself ("Q3 2027") shown as the group heading above — instead of one plain "Q3" column, so the picture doesn't rely on the reader knowing which months make up which quarter.
  - The exported chart also runs at a tighter date scale than the on-screen view — the live chart's spacing is generous because it needs room for a mouse to grab and resize a bar accurately, which an export doesn't need. This roughly halves the chart's width at every zoom level, so the picture stays a reasonable, slide-friendly shape instead of stretching into a long thin strip.
  - The **legend** at the bottom only lists categories actually used by a visible task in that export — categories you've defined in Settings but haven't assigned to anything in this plan are left out, so the picture doesn't advertise colors that don't appear anywhere in the chart.
